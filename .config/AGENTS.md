...
| `KNOWLEDGE_BASE.md` | deep-dive investigation write-ups (Electron/picom, USB-C hub/DP alt-mode) |
...
## Investigations (details in `KNOWLEDGE_BASE.md`)

Deep-dive write-ups live in `KNOWLEDGE_BASE.md`; read the entry before touching these areas:

- **Electron GPU & picom** (2026-09-16): don't re-add the removed Electron flags in startup.sh (`UseOzonePlatform`, `--use-gl`, `VaapiVideoDecoder` — dead/vestigial). `pkill picom` while Chromium/Electron apps run can silently downgrade them to software compositing until the app restarts. Discord's GPU spikes are app-side; `picom.conf` is tuned, leave it alone.
- **USB-C hub / DP alt-mode failure** (2026-09-16): bad boots leave the hub USB2-only with `DP-1-0` missing — signature: `lsusb -d 2109:0103` (billboard present). Software replug for peripherals = `authorized` toggle on `1-11`; monitor needs physical replug or suspend/resume (no kernel-side reset on this board). Boot-notify still unimplemented.
